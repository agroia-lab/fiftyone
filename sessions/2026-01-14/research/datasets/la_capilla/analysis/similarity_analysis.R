# La Capilla Similarity Network Analysis
...
neighbors_path <- "/home/malezainia1/dev/voxel51/fiftyone/sessions/2026-01-14/research/datasets/la_capilla/data/similarity_neighbors.csv"
...
p_dist <- ggplot(nn_distances, aes(x = distance, fill = pair_type)) +
  ...
  scale_fill_manual(values = c("Same Category" = "#27AE60", "Different Category" = "#E74C3C")) +
  labs(
    title = "Nearest Neighbor Distance Distribution - La Capilla",
...
ggsave(
  file.path(output_dir, "distance_distribution.png"),
...
p_network <- ggplot() +
  ...
  labs(
    title = "Image Similarity Network - La Capilla",
...
ggsave(
  file.path(output_dir, "similarity_network.png"),
  p_network, width = 12, height = 10, dpi = 150
)
cat("Saved: similarity_network.png\n")

# =============================================================================
# 3. Cluster Cohesion Analysis
# =============================================================================
...
p_cohesion <- ggplot(cohesion, aes(x = category, y = mean_dist, fill = category)) +
  ...
  scale_fill_manual(values = c("Very Close-up" = "#9B59B6", "Medium Altitude" = "#27AE60")) +
  labs(
    title = "Cluster Cohesion Analysis - La Capilla",
    subtitle = "Mean distance to 5 nearest neighbors by category",
    x = "",
...
ggsave(
  file.path(output_dir, "cluster_cohesion.png"),
  p_cohesion, width = 8, height = 6, dpi = 150
)
cat("Saved: cluster_cohesion.png\n")
...
cat(sprintf("Very close-up images: %d (%.1f%%)\n",
    sum(df$category == "Very Close-up"),
    100 * sum(df$category == "Very Close-up") / nrow(df)))
cat(sprintf("Medium altitude images: %d (%.1f%%)\n",
    sum(df$category == "Medium Altitude"),
    100 * sum(df$category == "Medium Altitude") / nrow(df)))
